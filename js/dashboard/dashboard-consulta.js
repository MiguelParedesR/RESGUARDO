...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const showMsg = (message) => {
        try { if (snackbar && snackbar.MaterialSnackbar) snackbar.MaterialSnackbar.showSnackbar({ message }); else alert(message); }
        catch { alert(message); }
    };
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key && e.key.toLowerCase() === 'printscreen') {
            ...
            setTimeout(() => antiOverlay && antiOverlay.classList.remove('show'), 1200);
    ...
    const selectClientesMobile = document.getElementById('select-clientes-mobile');
    ...
    function filterClientes(q) {
        ...
        if (!q) { renderSidebar(clientesCache); return; }
    ...
    async function cargarClientes() {
        try {
            ...
            if (selectClientesMobile) {
                selectClientesMobile.innerHTML = '<option value="">Seleccione cliente</option>';
    ...
    if (selectClientesMobile) {
        selectClientesMobile.addEventListener('change', async () => {
            ...
            if (!id) { placasContainer && (placasContainer.innerHTML = ''); clienteSeleccionado = null; return; }
            clienteSeleccionado = clientesCache.find(c => String(c.id) === String(id)) || null;
            placasContainer && (placasContainer.innerHTML = '');
    ...
    btnReset && btnReset.addEventListener('click', () => {
        clienteSeleccionado = null;
        placasContainer && (placasContainer.innerHTML = '');
        if (listaClientes) listaClientes.querySelectorAll('li').forEach(li => li.classList.remove('is-active'));
