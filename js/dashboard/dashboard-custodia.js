...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function findActiveService(empresa, placa) { const { data, error } = await window.sb.from('servicio').select('id, empresa, placa, tipo, estado, destino_texto, cliente:cliente_id(nombre)').eq('empresa', empresa).eq('placa', placa).eq('estado', 'ACTIVO').order('created_at', { ascending: false }).limit(1); if (error) throw error; return (data && data[0]) || null; }
