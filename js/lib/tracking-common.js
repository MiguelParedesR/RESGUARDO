...
(function () {
  ...
  function drawRouteWithPOIs(map, layer, start, dest, routeLatLngs) {
    try { if (layer) layer.clearLayers(); } catch {}
    const { pinUser, pinDest } = buildIcons();
    const lg = layer || L.layerGroup().addTo(map);
    L.marker(start, { icon: pinUser, title: 'Partida/Actual' }).bindTooltip('Partida/Actual').addTo(lg);
    if (dest) L.marker(dest, { icon: pinDest, title: 'Destino' }).bindTooltip('Destino').addTo(lg);
    if (routeLatLngs && routeLatLngs.length) {
      L.polyline(routeLatLngs, { color: '#1e88e5', weight: 4, opacity: 0.95 }).addTo(lg);
      map.fitBounds(L.latLngBounds(routeLatLngs), { padding: [40, 40], maxZoom: 16 });
    } else if (dest) {
      L.polyline([start, dest], { color: '#455a64', weight: 3, opacity: 0.85, dashArray: '6,4' }).addTo(lg);
      map.fitBounds(L.latLngBounds([start, dest]), { padding: [40, 40], maxZoom: 16 });
    } else {
  ...
  async function routeLocal(start, dest) {
    if (!window.routerLocal || typeof window.routerLocal.route !== 'function') return null;
  ...
  function beep() {
    try {
      ...
      o.start();
      setTimeout(() => { g.gain.exponentialRampToValueAtTime(0.0001, ctx.currentTime + 0.15); o.stop(ctx.currentTime + 0.2); }, 160);
    } catch {}
  }
