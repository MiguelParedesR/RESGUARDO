(function (global) {
  ...
  const state = {
    ...
    subscribers: new Set(),
    config: {
      vapidPublicKey: (global.APP_CONFIG && global.APP_CONFIG.WEB_PUSH_PUBLIC_KEY) || ''
    },
    custodia: {
      lastLocation: null
    },
  ...
  function log() { try { console.log('[Alarma]', ...arguments); } catch (_) { } }
  function warn() { try { console.warn('[Alarma]', ...arguments); } catch (_) { } }
  function error() { try { console.error('[Alarma]', ...arguments); } catch (_) { } }
  ...
  function initAdmin(options) {
    ...
    setupEscShortcuts();
    if (options && typeof options.enrichEvent === 'function') state.admin.enrichEvent = options.enrichEvent;
  }
  ...
  async function emit(type, payload) {
    ...
    if (!client) {
      ...
      queueOffline(record);
      notify({ type: 'emit', status: 'queued', event: sanitized, source: 'offline' });
      return { queued: true };
    }
    try {
      ...
      queueOffline(record);
      notify({ type: 'emit', status: 'queued', event: sanitized, source: 'error', error: err });
  ...
  async function triggerPush(type, eventRecord, options) {
    ...
    const custom = {};
    if (type === 'start') custom.role = 'admin';
    if (type === 'panic') custom.role = 'admin';
    if (type === 'checkin_ok' || type === 'checkin_missed') custom.role = 'custodia';
  ...
  function sirenaOff() {
    ...
    state.siren.voices.forEach(({ osc, gain }) => {
      try {
        if (gain) gain.gain.exponentialRampToValueAtTime(0.0001, (gain.context || osc.context).currentTime + 0.05);
      } catch (_) { }
  ...
  function tts(text, options) {
    ...
    utter.onend = () => { state.admin.ttsLock = false; };
    try { global.speechSynthesis.speak(utter); } catch (err) { state.admin.ttsLock = false; warn('No se pudo reproducir TTS', err); }
  }
  ...
  function ensureModal() {
    ...
    buttons.close?.addEventListener('click', () => { closePanicModal(); });
    buttons.silence?.addEventListener('click', () => { sirenaOff(); });
    ...
    buttons.voiceConfirm?.addEventListener('click', () => {
      ...
      if (!input) return;
      if (String(input.value || '').toLowerCase().includes('silenciar')) {
        sirenaOff();
        closePanicModal();
        markPanicHandled();
  ...
  function openPanicModal(record) {
    ...
    if (body) {
      ...
      rows.forEach((row) => {
        ...
        div.innerHTML = `<strong>${clip(row.label, 32)}</strong><span>${clip(row.value, MAX_STRING) || '-'}</span>`;
  ...
  function formatTime(timestamp) {
    ...
    try {
      ...
      return date.toLocaleString('es-PE', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
  ...
  function startVoiceRecognition() {
    ...
    if (!Recognition) {
      if (status) status.textContent = 'Reconocimiento de voz no soportado. Usa el formulario.';
    ...
    try {
      ...
      recog.onerror = (event) => {
        ...
        if (status) status.textContent = 'No se pudo escuchar. Usa el formulario manual.';
        if (fallback) fallback.hidden = false;
      };
      ...
      if (status) status.textContent = 'Error al iniciar reconocimiento. Usa el formulario.';
      if (fallback) fallback.hidden = false;
    }
  }
  ...
  function stopVoiceRecognition() {
    if (state.admin.voice.recognition) {
      try { state.admin.voice.recognition.onend = null; state.admin.voice.recognition.stop(); } catch (_) { }
  ...
  function showToast(message) {
    ...
    toast._timerId = setTimeout(() => { toast.hidden = true; }, TOAST_TIMEOUT);
  }

  function registerPush(role, empresa, metadata) {
    return (async () => {
      if (!('Notification' in global)) throw new Error('Notifications no soportadas');
      if (!navigator.serviceWorker) throw new Error('Service worker requerido para push');
  ...
  function openCheckinPrompt(payload) {
    ...
    try { navigator.vibrate?.([160, 60, 160]); } catch (_) { }
  }
  ...
  function ensureCheckinPanel() {
    ...
    panel.querySelector('.js-checkin-close')?.addEventListener('click', () => panel.classList.remove('is-open'));
    panel.querySelector('.js-checkin-confirm')?.addEventListener('click', () => confirmCheckin(panel));
    panel.querySelector('.js-checkin-voice')?.addEventListener('click', () => captureCheckinVoice(panel));
  ...
  function captureCheckinVoice(panel) {
    ...
    if (button) { button.disabled = true; button.textContent = 'Escuchando...'; }
    try {
      const recog = new Recognition();
      recog.lang = 'es-PE';
      recog.continuous = false;
      ...
      recog.onerror = () => { showToast('No se pudo capturar tu voz. Escribe la respuesta.'); };
      recog.onend = () => {
        if (button) { button.disabled = false; button.textContent = 'Responder por voz'; }
      };
      recog.start();
    } catch (err) {
