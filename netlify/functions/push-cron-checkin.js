...
export async function handler(event) {
  ...
  const resendDelayMinutes = Number.parseInt(CHECKIN_REMINDER_MIN_INTERVAL, 10) || 5;
  ...
  const { data: services, error: svcError } = await supabase
    .from('servicio')
    .select('id, empresa, placa, tipo, estado, last_checkin_at, cliente:cliente_id(nombre)')
    .eq('estado', 'ACTIVO');
  ...
  for (const svc of services || []) {
    try {
      const lastCheckIn = svc.last_checkin_at ? new Date(svc.last_checkin_at) : null;
      const diff = lastCheckIn ? minutesDiff(now, lastCheckIn) : thresholdMinutes + 1;
      ...
      const lastReminderAt = reminders?.[0]?.created_at ? new Date(reminders[0].created_at) : null;
      if (lastReminderAt && minutesDiff(now, lastReminderAt) < resendDelayMinutes) {
        continue;
      }
